  ...
  if (method === 'DELETE' && batchId) {
    ...
    if (path === '/display-settings') {
      ...
      if (method === 'PUT') {
        if (!requireMainAdmin()) return jsonResponse({ error: 'Forbidden' }, 403);
        const body = await req.json();
        if (!['classic', 'pulse', 'broadcast', 'aurora-light', 'aurora-dark', 'ocean-light', 'ocean-dark', 'sunset-light', 'sunset-dark', 'orbit-light', 'orbit-dark', 'blueprint-light', 'blueprint-dark', 'editorial-light', 'editorial-dark'].includes(body?.theme)) {
