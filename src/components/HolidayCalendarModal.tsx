...
export function HolidayCalendarModal({ open, onOpenChange }: HolidayCalendarModalProps) {
  ...
  useEffect(() => {
    if (!open) return;
    setStatus('loading');
    supabase.from('holidays').select('date, title, is_national').order('date').then(({ data, error }) => {
  ...
  return (
    <section className="schedule-page-panel">
      <div className="grid gap-5 min-w-0">{status === 'loading' && <p role="status">Loading holidays…</p>}{status === 'error' && <p role="alert">Could not load holidays. <Button variant="outline" onClick={()=>setRetry(n=>n+1)}>Retry</Button></p>}
        <div>
        ...
        {status === 'ready' && monthHolidays.length === 0 && <p className="rounded-lg bg-muted p-4 text-sm text-muted-foreground">No holidays listed for this month. Friday and Saturday are weekends.</p>}
