...
export function ScheduleGrid({ schedule }: ScheduleGridProps) {
  const [mobileDay, setMobileDay] = useState<string>(DAYS.includes(new Date().toLocaleDateString('en-US', { weekday: 'long' }) as typeof DAYS[number]) ? new Date().toLocaleDateString('en-US', { weekday: 'long' }) : DAYS[0]);
  ...
  return (
    <div className="overflow-x-auto pb-4 routine-grid"><label className="md:hidden block mb-4 text-sm font-medium">Day<select aria-label="Schedule day" value={mobileDay} onChange={e=>setMobileDay(e.target.value)} className="ml-3 rounded-lg border bg-card p-3">{DAYS.map(day=><option key={day}>{day}</option>)}</select></label>
        ...
        <thead>
          <tr>
            ...
            {DAYS.map((day) => (
              <th
                key={day}
                data-mobile-hidden={day !== mobileDay}
          ...
          {timeSlots.map((timeSlot) => (
            <tr key={timeSlot.index} className={cn(timeSlot.isBreak && 'opacity-75')}>
              ...
              {DAYS.map((day) => {
                ...
                return (
                  <td data-mobile-hidden={day !== mobileDay} key={cellId} className="align-middle min-w-[135px] sm:min-w-[145px] md:min-w-[150px]">
                    <ScheduleCell
                      cellSlots={cellSlots}
