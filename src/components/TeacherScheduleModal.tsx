...
import { generateTeacherSchedulePDF } from '@/utils/teacherSchedulePdf';
...
const ADMIN_API_URL =
  'https://wkvicmgacjdoraoufprw.supabase.co/functions/v1/admin';
...
export function TeacherScheduleModal({
  ...
  const [mobileDay, setMobileDay] = useState('Sunday');
  ...
  const fetchTeachers = async () => {
    setDirectoryError(false); setDirectoryLoading(true);
    try {
    const res = await fetch(`${ADMIN_API_URL}/teachers`);
    ...
    } catch { setDirectoryError(true); } finally { setDirectoryLoading(false); }
  };
  ...
  return (
    <section className="schedule-page-panel">
      <div className="grid gap-5 min-w-0">
        {directoryLoading && <p role="status">Loading directory…</p>}
        {directoryError && <p role="alert">Could not load the directory. <Button variant="outline" onClick={fetchTeachers}>Retry</Button></p>}
        <div>
          <div className="flex items-center justify-between w-full">
            <h2 className="text-xl font-bold">Teacher Schedule</h2>
        ...
        <label className="md:hidden text-sm font-medium">Day<select aria-label="Schedule day" value={mobileDay} onChange={e=>setMobileDay(e.target.value)} className="ml-3 rounded-lg border bg-card p-3">{DAYS.map(day=><option key={day}>{day}</option>)}</select></label>
        {/* ================= SCHEDULE ================= */}
        {(
            ...
            >
              ...
              <div className="w-full min-w-full sm:min-w-max md:min-w-full">
                ...
                >
                  ...
                  {DAYS.map((day) => (
                    <div
                      key={day} data-mobile-hidden={day !== mobileDay}
                ...
                <div className="grid auto-rows-min">
                  ...
                  <div className="hidden sm:block">
                    ...
                    >
                      ...
                      {DAYS.map((day) => (
                        <div key={day} data-mobile-hidden={day !== mobileDay} className="border-r p-2 bg-gray-50 dark:bg-muted/20" />
                      ))}
                    </div>
                  </div>
                </div>
