...
export default function CoAdminsPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newUsername.trim() || newPassword.length < 10) {
      toast({ title: 'Invalid input', description: 'Username and 10+ character password required', variant: 'destructive' });
      return;
    }
    if (!newBatchName) {
  ...
  const doResetPassword = async () => {
    if (!passwordFor || resetPass.length < 10) return;
    setBusy(true);
    try {
        ...
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleCreate} className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">Minimum 10 characters. Share it privately with the co-admin.</p>
            </div>
            <div className="space-y-1.5">
              <Label>Batch</Label>
      ...
      <Dialog open={!!passwordFor} onOpenChange={(o) => !o && !busy && (setPasswordFor(null), setResetPass(''))}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Minimum 10 characters. Existing sessions will be signed out.</p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setPasswordFor(null); setResetPass(''); }} disabled={busy}>Cancel</Button>
            <Button onClick={doResetPassword} disabled={busy || resetPass.length < 10}>
