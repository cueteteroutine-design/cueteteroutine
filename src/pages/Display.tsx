import { isDisplayTheme, displayMode } from '@/lib/displayThemes';
...
import { getDisplaySettings, type DisplayTheme } from '@/lib/adminApi';
...
const Display = () => {
  ...
  const [displayTheme, setDisplayTheme] = useState<DisplayTheme>('classic');
  const { theme: appTheme } = useTheme();
  useEffect(() => {
    const root = document.documentElement;
    root.classList.toggle('dark', displayMode(displayTheme) === 'dark');
    root.classList.toggle('light', displayMode(displayTheme) === 'light');
    return () => { root.classList.toggle('dark', appTheme === 'dark'); root.classList.toggle('light', appTheme === 'light'); };
  ...
  useEffect(() => {
    ...
    const loadTheme = async () => {
      try {
        ...
        if (active) setDisplayTheme(isDisplayTheme(settings.theme) ? settings.theme : 'classic');
      } catch (error) {
        console.error('Failed to load display theme:', error);
      }
    };
    void loadTheme();
    // Poll as a fallback if the screen loses its realtime connection.
    const timer = setInterval(loadTheme, 30000);
    const channel = supabase.channel(`display-theme-${crypto.randomUUID()}`)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'display_settings' }, loadTheme)
      .subscribe();
    return () => {
      active = false;
      clearInterval(timer);
  ...
  // Set up realtime subscription for batches table
  useEffect(() => {
    console.log('Setting up realtime subscription for batches...');
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Refetch all batches when any change occurs
          fetchBatches();
        }
      )
      .subscribe((status) => {
  ...
  return (
    <div className={`display-theme display-theme-${displayTheme} display-style-${displayTheme.split('-')[0]} h-screen w-screen bg-background flex flex-col overflow-hidden`}>
      ...
      <main className="flex-1 min-h-0 overflow-y-auto">
        <div className="display-card-list h-full w-full flex flex-col p-2 sm:p-3 md:p-4 gap-2 sm:gap-3">
          {batches.length === 0 ? (
